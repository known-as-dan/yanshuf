...
function buildPhotoFilename(
	...
	if (entry.sectionCode) {
		const shortName =
			SHORT_NAMES[entry.sectionCode] ?? sanitizeFilename(entry.description).slice(0, 15);
...
export async function downloadAllZip(
	...
	// Phase 1: Build Excel buffer
	onProgress?.({ phase: 'excel', current: 0, total: 1 });
	const { buffer: excelBuffer, result } = await buildWorkbookBuffer(inspection, allDefects);
	onProgress?.({ phase: 'excel', current: 1, total: 1 });
	...
	if (entries.length > 0) {
		...
		for (let i = 0; i < entries.length; i++) {
			...
			onProgress?.({ phase: 'photos', current: i + 1, total: entries.length });
	...
	const zipBlob = await zip.generateAsync({ type: 'blob' });
	onProgress?.({ phase: 'zip', current: 1, total: 1 });
