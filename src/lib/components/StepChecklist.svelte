<script lang="ts">
	...
	let sectionStats = $derived.by(() => {
		...
		return {
			...
			totalFilled,
			totalFaults,
			totalItems: store.inspection.checklist.length
		};
	});
...
{#if activeSection}
	...
	<div class="pointer-events-none fixed inset-x-0 top-0 z-40 flex justify-center px-4 pt-2">
		<div
			class="flex items-center gap-2 rounded-xl border {activeColors.border} bg-surface-800/95 px-3 py-2 shadow-lg shadow-black/30 backdrop-blur-md"
		>
			<span class="text-lg">{sectionIcons[activeSection]}</span>
			<span class="text-sm font-semibold text-white"
				>{activeSectionData.code}. {activeSectionData.title}</span
			>
			{#if activeStat.faults > 0}
				<span class="rounded-md bg-danger-dim px-2 py-0.5 text-xs font-medium text-danger"
					>{activeStat.faults} תקלות</span
				>
			{/if}
			<span class="rounded-md bg-surface-600 px-2 py-0.5 text-xs font-medium text-gray-400"
				>{activeStat.filled}/{activeStat.total}</span
			>
		</div>
	</div>
{/if}
...
<div class="space-y-4">
	<div class="flex items-center justify-between">
		<h2 class="text-lg font-bold text-white lg:text-xl">סעיפי בדיקה</h2>
		<div class="flex items-center gap-2">
			{#if sectionStats.totalFaults > 0}
				<div
					class="flex items-center gap-1.5 rounded-full bg-danger-dim px-3 py-1.5 text-sm font-semibold text-danger"
				>
			...
			{/if}
			<div
				class="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-semibold {sectionStats.totalFilled ===
					sectionStats.totalItems && sectionStats.totalFaults === 0
					? 'bg-ok-dim text-ok'
			...
			>
				<div
					class="h-2 w-2 rounded-full {sectionStats.totalFilled === sectionStats.totalItems &&
					sectionStats.totalFaults === 0
						? 'bg-ok'
						: sectionStats.totalFilled === sectionStats.totalItems
							? 'bg-warn'
							: 'bg-accent'}"
	...
	{#each checklistSections as section (section.code)}
		...
		<div
			bind:this={sectionRefs[section.code]}
			class="overflow-hidden rounded-xl border {faults > 0
				? 'border-danger/30'
		...
		>
			<div class="flex items-center justify-between p-3 lg:p-4">
				...
				>
					<span class="flex h-8 w-8 items-center justify-center rounded-lg text-lg {colors.icon}"
						>{sectionIcons[section.code] || '📋'}</span
					>
					<div>
						<span class="font-semibold text-white">{section.code}. {section.title}</span>
				...
				<div class="flex items-center gap-2">
					{#if faults > 0}
						<span
							class="rounded-md bg-danger-dim px-2 py-0.5 text-xs font-medium text-danger lg:text-sm"
						>
							{faults} תקלות
						</span>
					{/if}
					<span
						class="rounded-md px-2 py-0.5 text-xs font-medium lg:text-sm {allOk
							? 'bg-ok-dim text-ok'
							: complete
								? 'bg-warn-dim text-warn'
			...
			{#if expandedSections[section.code]}
				<div class="border-t border-border" transition:slide={{ duration: 300 }}>
					{#each section.items as item, idx (item.sectionCode)}
						...
						{#if showSubgroup}
							<div
								class="border-t-2 {colors.border} {colors.bg} px-4 py-2.5 {idx > 0 ? 'mt-1' : ''}"
							>
								<span class="text-sm font-bold {colors.text}">{item.subgroup}</span>
							</div>
						{/if}
						<div
							class="border-b border-border/50 p-3 last:border-b-0 lg:p-4 {checklist?.status ===
							'לא תקין'
								? 'bg-danger-dim/50'
						...
						>
							<div class="lg:flex lg:items-start lg:gap-4">
								<div class="mb-2 flex items-start gap-2 lg:mb-0 lg:flex-1">
									<span
										class="mt-0.5 rounded bg-surface-600 px-1.5 py-0.5 font-mono text-xs text-gray-400 lg:text-sm"
										>{item.sectionCode}</span
									>
									<span
										class="text-sm lg:text-base {checklist?.status === 'לא רלוונטי'
											? 'text-gray-500 line-through'
											: 'text-gray-200'}">{item.description}</span
									>
								</div>
								<div class="flex flex-wrap gap-1.5 lg:flex-shrink-0">
									{#if item.selectOptions}
										{#each item.selectOptions as opt (opt)}
											...
											<label
												class="flex cursor-pointer items-center gap-1 rounded-lg border px-3 py-1.5 text-sm transition-all lg:px-4 lg:py-2 lg:text-base {selected
											...
											>
												<input
													...
													class="sr-only"
													checked={selected}
													onchange={() => store.updateChecklistItem(item.sectionCode, opt)}
												/>
												{#if selected}✓{/if}
												{opt}
											</label>
										...
										<label
											class="flex cursor-pointer items-center gap-1 rounded-lg border px-3 py-1.5 text-sm transition-all lg:px-4 lg:py-2 lg:text-base {lrSelected
												? 'border-accent bg-accent-dim text-accent'
												: 'border-border text-gray-400 hover:border-border-light hover:text-gray-300 active:border-border-light active:text-gray-300'}"
										>
											<input
										...
										{#each options as opt (opt.label)}
											...
											>
												<input
													...
													class="sr-only"
													checked={selected}
													onchange={() => store.updateChecklistItem(item.sectionCode, opt.label)}
												/>
							...
							<div class="mt-2 flex items-center rounded-lg bg-surface-700">
								<input
									...
									oninput={(e) =>
										store.updateChecklistItem(item.sectionCode, undefined, e.currentTarget.value)}
								/>
								{#if !photos.length}
									<PhotoCapture
										compact
...
{#if jumpMenuOpen}
	<div
		class="fixed start-4 bottom-40 z-50 w-64 overflow-hidden rounded-xl border border-border bg-surface-800 shadow-xl shadow-black/40 lg:start-[max(1rem,calc((100vw-66rem)/2))] lg:bottom-[10.5rem] landscape:bottom-36 lg:landscape:bottom-[10.5rem]"
	...
	>
		{#each checklistSections as section (section.code)}
			...
			>
				<span
					class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg text-base {colors.icon}"
				>
					{sectionIcons[section.code] || '📋'}
				</span>
				<span class="flex-1 text-sm leading-tight font-medium text-white"
					>{section.code}. {section.title}</span
				>
				<span
					class="text-xs font-medium {allOk ? 'text-ok' : complete ? 'text-warn' : 'text-gray-500'}"
				>
					{filled}/{total}
				</span>
				{#if faults > 0}
...
<button
	type="button"
	class="fixed start-4 bottom-24 z-50 flex h-12 w-12 items-center justify-center rounded-2xl bg-accent text-white shadow-lg shadow-black/30 transition-transform active:scale-95 lg:start-[max(1rem,calc((100vw-66rem)/2))] lg:bottom-[6.5rem] landscape:bottom-20 lg:landscape:bottom-[6.5rem]"
	onclick={() => {
		haptic('light');
...
>
	{#if jumpMenuOpen}
		<svg class="h-[22px] w-[22px]" viewBox="0 -960 960 960" fill="currentColor"
			><path
				d="M480-424 284-620q-11-11-28-11t-28 11q-11 11-11 28t11 28l224 224q12 12 28 12t28-12l224-224q11-11 11-28t-11-28q-11-11-28-11t-28 11L480-424Z"
				transform="rotate(45, 480, -480)"
			/></svg
		>
	{:else}
		<svg class="h-[22px] w-[22px]" viewBox="0 -960 960 960" fill="currentColor"
